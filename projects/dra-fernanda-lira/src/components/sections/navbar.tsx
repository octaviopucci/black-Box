  ...
  return (
    <>
      <header className="pointer-events-none fixed inset-x-0 top-0 z-40 flex justify-center px-4 pt-6">
        <nav className="pointer-events-auto flex w-full max-w-5xl items-center justify-between rounded-full bg-elevated/70 px-4 py-2.5 shadow-[0_8px_32px_rgba(28,20,16,0.06)] ring-1 ring-ink/[0.06] backdrop-blur-xl">
          <button
            type="button"
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-30 bg-paper/90 backdrop-blur-3xl md:hidden"
          >
