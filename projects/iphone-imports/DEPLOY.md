...
O build gera `out/` (site + gestor) e `api/iphone-imports.js` (API).
Config: `outputDirectory: out` + `functions.api/iphone-imports.js` + `rewrites`.
**Não use `builds[]` nem Build Output API** — causam 404 no site e na API.
