import { createAdminCategoryPersisted, listAdminCategories } from "@/lib/store";
...
export async function POST(req: Request) {
  const auth = requireAdmin(req);
  if (!auth) return Response.json({ error: "Não autorizado." }, { status: 401 });
  const result = await createAdminCategoryPersisted(auth.establishment.id, await readJson(req));
  if ("error" in result) {
