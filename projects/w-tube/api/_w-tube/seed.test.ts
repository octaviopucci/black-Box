 ...
 * Rodar: npx tsx api/_w-tube/seed.test.ts
 */
...
async function run() {
  ...
  const custom: OrgDatabase = {
    ...
    products: [
      ...first.products,
      {
        ...
        slug: 'w-tube-custom-gestor',
        name: 'Produto Custom Gestor',
        brand: 'W-Tube',
        description: 'Teste',
  ...
  assert.equal(db.products.some((p) => p.slug === 'w-tube-custom-gestor'), true, 'produto do gestor preservado')
