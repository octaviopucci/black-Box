    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="absolute left-0 top-0 bottom-0 w-[min(320px,88vw)] overflow-y-auto bg-brand-black text-white">
        ...
        <nav className="px-4 pb-8">
          <p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-brand-purple">
            Categorias
          </p>
            ...
            {categories.map((cat) => (
              <li key={cat.slug}>
                <Link
                  ...
                  onClick={onClose}
                  className="block rounded-lg px-3 py-2.5 text-sm font-medium text-white/80 transition-colors hover:bg-brand-purple/10 hover:text-brand-purple"
                >
                  {cat.name}
                </Link>
              </li>
