...
function PeriodRow({
  ...
  return (
    <tr className="border-t border-brand-border hover:bg-white/[0.02]">
      ...
      <td className="p-3 text-right tabular-nums font-semibold text-brand-purple">{formatCurrency(revenue)}</td>
          ...
          {heroPeriods.map((p) => {
            ...
            return (
              <div key={p.id} className="card">
                ...
                <p className="mt-2 text-xl font-black text-brand-purple">{formatCurrency(p.revenue)}</p>
          ...
          <div className="flex h-48 items-end gap-1">
            {totals.monthlyRevenue.map((m) => (
              <div key={m.month} className="flex flex-1 flex-col items-center gap-1">
                <div
                  className="w-full rounded-t bg-gradient-to-t from-brand-muted to-brand-purple transition-all"
              ...
              {totals.recentSales.length === 0 ? (
                ...
                totals.recentSales.map((sale) => (
                  <tr key={sale.unitId} className="border-t border-brand-border">
                    ...
                    <td className="p-3 text-right font-semibold text-brand-purple">
