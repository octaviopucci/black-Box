...
export function setActiveWhatsapp(phone: string) {
  activeWhatsapp = phone.replace(/\D/g, "") || String(site.whatsapp);
}

export function getActiveWhatsapp() {
  return activeWhatsapp;
}
...
export const defaultMessage = `Olá! Vim pelo site da ${site.name} e quero saber mais sobre os carros disponíveis.`;

export function whatsappHref(message: string = defaultMessage) {
  return `https://wa.me/${getActiveWhatsapp()}?text=${encodeURIComponent(message)}`;
}
