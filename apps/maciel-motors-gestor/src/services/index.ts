...
export const backupService = {
  exportJSON(): void {
    ...
    downloadJSON(`maciel-motors-backup-${stamp}.json`, db)
  },
...
export const dashboardService = {
  getStats(): DashboardStats {
    ...
    const diasMediosEstoque = stock.length
      ? stock.reduce((acc, v) => acc + daysBetween(v.dataCompra), 0) / stock.length
      : 0
...
export const reportService = {
  ...
  exportPDF(title: string, rows: string[][]) {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>${title}</title>
      ...
      <h1>${title}</h1>
      <p>Maciel Motors Gestor — ${new Date().toLocaleString('pt-BR')}</p>
      <table>${table}</table>
