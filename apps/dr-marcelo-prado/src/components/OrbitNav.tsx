          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span className="relative block h-3 w-5">
                <span className={`absolute left-0 h-px w-full bg-paper transition ${open ? 'top-1.5 rotate-45' : 'top-0'}`} />
                <span className={`absolute left-0 top-1.5 h-px w-full bg-paper transition ${open ? 'opacity-0' : ''}`} />
                <span className={`absolute left-0 h-px w-full bg-paper transition ${open ? 'top-1.5 -rotate-45' : 'top-3'}`} />
              </span>
            </button>
          </div>
        </motion.div>
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 flex flex-col justify-end bg-void/95 px-6 pb-12 pt-28 lg:hidden"
          >
            {links.map((l, i) => (
              <motion.a
                key={l.href}
