...
export function Approach() {
  return (
    <section id="modulacao" className="relative overflow-hidden py-24 sm:py-32">
      <div className="pointer-events-none absolute -left-24 top-16 h-72 w-72 rounded-full bg-aqua/10 blur-3xl" />
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="grid gap-14 lg:grid-cols-[1.15fr_0.85fr] lg:gap-20">
          <div>
            <Reveal>
              <SectionEyebrow>Modulação</SectionEyebrow>
              <h2 className="font-display text-[clamp(2.2rem,5vw,3.5rem)] font-semibold leading-[1.05] tracking-tight text-ink text-balance">
              ...
              {site.principles.map((item, i) => (
                <Reveal key={item.title} delay={0.08 * i}>
                  <article className="grid grid-cols-[auto_1fr] gap-5 border-t border-line py-7 last:border-b">
                    <span className="font-display text-3xl font-semibold text-aqua/70">
          ...
          <Reveal delay={0.12} className="relative">
            <div className="sticky top-28 overflow-hidden rounded-[2rem] bg-ink p-8 text-snow shadow-lift sm:p-10">
              <div className="absolute -right-12 -top-12 h-44 w-44 rounded-full bg-aqua/40 blur-3xl" />
              <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-aqua-light">
                Promessa clínica
              </p>
              ...
              <div className="mt-10 flex items-center gap-4 border-t border-snow/10 pt-8">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-aqua font-display text-sm font-semibold">
                  {site.shortName}
                </div>
                <div>
