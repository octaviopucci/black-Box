    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
        <a href="#topo" className="group flex items-center gap-2.5 sm:gap-3" aria-label={site.fullName}>
          <LogoImage className="h-8 w-8 transition duration-500 group-hover:scale-105 sm:h-9 sm:w-9" />
          <span
            className={`font-display text-xl font-semibold tracking-[0.04em] transition-colors duration-500 sm:text-2xl ${
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <a
            ...
            className={`rounded-full p-2 transition ${
              onDark
                ? 'text-cream/70 hover:text-rose-soft'
          ...
          <a href={whatsappUrl()} className={onDark ? 'cta-wine bg-rose text-ink hover:bg-rose-soft' : 'cta-wine'} data-cursor>
            Agendar
          </a>
        </div>
      ...
      {open && (
        ...
        >
          ...
          <a href={whatsappUrl()} className="cta-wine mt-8" onClick={() => setOpen(false)}>
