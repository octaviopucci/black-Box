...
export function Essence() {
  return (
    <section id="essencia" className="relative overflow-hidden py-24 sm:py-32">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-vida/50 to-transparent"
      />
      <svg
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-16 w-[140%] -translate-x-1/2 opacity-35"
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="mt-4 grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          ...
          <Reveal delay={0.2}>
            <div className="space-y-5 border-l border-vida/40 pl-6 text-base leading-relaxed text-mute sm:text-lg">
              {site.story.map((p) => (
                <p key={p}>{p}</p>
              ))}
            </div>
        ...
        <div className="mt-16 grid gap-10 border-t border-ink/8 pt-12 sm:grid-cols-3 sm:gap-8">
          {site.principles.map((item, i) => (
            <Reveal key={item.title} delay={0.08 * i}>
              <article>
                <p className="font-display text-4xl text-vida/60">0{i + 1}</p>
